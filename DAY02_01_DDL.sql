...
CREATE TABLE CUSTOMER_T (
    ...
    CUST_AGE    NUMBER(3)           CHECK(CUST_AGE >= 0 AND CUST_AGE <=120), --어차피 나이는 3자리를 안넘어가니까 데이터 아끼려면
                     -- CHECK(CUST_AGE BETWEEN 0 AND 120),  위와 동일한 방법
    BANK_CODE   VARCHAR2(20 BYTE), --부모키랑 같은 타입 부모가 VARCHAR2타입이면 자식도 VARCHAR2타입으로
...
-- 1. BANK_T 에 연락처 (BANK_TEL) 칼럼을 추가하시오.
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(15 BYTE);

-- 2. BANK_T의 은행명(BANK_NAME) 칼럼을 VARCHAR2(15 BYTE)로 바꾸고 필수 제약조건을 추가하시오. NOT NULL 추가
...
-- 5. CUSTOMER_T의 등급(GRADE) 칼럼을 추가하시오. 'VIP', 'GOLD', 'SILVER', 'BRONZE' 중 하나의 값을 가지도록 설정하시오.
ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIP','GOLD','SILVER','BRONZE'));
...
/*
    ...
    2.FK
        ...
        2) 삭제      : ALTER TABLE 테이블명      DROP    CONSTRAINT 제악조건명
