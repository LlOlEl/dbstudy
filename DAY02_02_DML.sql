...
-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서를 삭제하시오. (부서번호가 3인 사원은 없다)
DELETE 
  FROM department_T
 WHERE DEPT_NO = 3;
...
-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)가 2인 부서를 삭제하시오 (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 
--      참조 무결성 위배에 대비하여 ON DELETE SET NULL 옵션을 주었으므로 사원 테이블의 부서번호가 NULL로 함꼐 수전된다
